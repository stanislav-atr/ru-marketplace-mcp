...
uv run pytest -q -m "not live and not cdp"   # 1932 офлайн-тестов, сеть не нужна
```
...
```bash
uv sync --all-packages
uv run pytest -q -m "not live and not cdp"    # 1932 офлайн-тестов
...
ошибаются уверенно, поэтому проект устроен вокруг проверки: 1932 офлайн-тестов,
...
cd ru-marketplace-mcp
uv sync --all-packages
uv run pytest -q -m "not live and not cdp"    # 1932 offline tests, no network needed
```
...
```bash
uv sync --all-packages
uv run pytest -q -m "not live and not cdp"    # 1932 offline tests
...
are confidently wrong, so the project is arranged around verification: 1932 offline
